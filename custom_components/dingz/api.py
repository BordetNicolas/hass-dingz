...
class Client:
    ...
    def __init__(
        ...
        self._lock = _ReqThrottleLock(REQUEST_THROTTLE)
    ...
    async def _get(
        ...
        *,
        ...
        retry_delay: float = GET_RETRY_DELAY,
    ...
    async def _post(
        ...
        *,
        ...
        retry_delay: float = POST_RETRY_DELAY,
    ) -> None:
        url = self._base_url / "api/v1" / path
        ...
        async def once() -> None:
            _LOGGER.debug(
                "post to %s with payload %s", url, _payload_for_log(path, data)
            )
...
async def _repeat(
    ...
    for _ in range(attempts):
        try:
            ...
            _LOGGER.debug("client error: %s", exc)
            last_exc = exc
...
def _payload_for_log(path: str, data: dict[str, Any] | str) -> object:
    """Avoid logging MQTT credentials embedded in services_config."""
    if path == "services_config":
        return "<redacted>"
    return data
